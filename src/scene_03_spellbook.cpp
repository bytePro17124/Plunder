...
void SceneManager::scene_03_spellbook() {
	...
	while (scene == SPELLBOOK_CREATE) {    // scene loop
		...
		if (spellbookReadyToDisplay) {  //Spellbook created
			...
			save_loot_button.draw(ScreenWidth - 75 - save_loot_button.getWidth(),
								  ScreenHeight - 75 - save_loot_button.getHeight());
			...
			if (loot_write_out) {
				...
				fileoutputname =
						"saved_loot/spellbook" + std::to_string(1900 + ltm->tm_year) + std::to_string(1 + ltm->tm_mon) +
						std::to_string(ltm->tm_mday) + "_" + std::to_string(ltm->tm_hour) +
						std::to_string(ltm->tm_min) + std::to_string(ltm->tm_sec) + "_" +
						std::to_string(loot_rolls_this_session) + ".txt";
			...
			if (loot_write_out_notification) {
				...
				notification_text.draw(ScreenWidth / 2 - notification_text.getWidth() / 2,
									   ScreenHeight - notification_text.getHeight() - 10);
				frame_count++;
			}


		} else {               //Spellbook has not been created yet
			spellbook_scene_header.draw(ScreenWidth / 2 - spellbook_scene_header.getWidth() / 2,
										spellbook_scene_header.getHeight()); //title text
			...
			if (hasSpells)
				pagesUsedDisplay.draw(
						&spellbook_pages_used_draw);  //display number of pages used in current spellbook setup
			if (hasSpells && hasDescription &&
				hasValidTotalPages) { //draw the makeSpellbook button if all the input is currently sufficient
