...
void SceneManager::scene_02_hoard_loot() {
	...
	while (scene == HOARD_LOOT) {    // scene loop
		...
		if (loot_write_out) {
			now = time(nullptr);            //update time to now
			ltm = localtime(&now);  //save time into struct
			fileoutputname =
					"saved_loot/lootroll" + std::to_string(1900 + ltm->tm_year) + std::to_string(1 + ltm->tm_mon) +
					std::to_string(ltm->tm_mday) + "_" + std::to_string(ltm->tm_hour) + std::to_string(ltm->tm_min) +
					std::to_string(ltm->tm_sec) + "_" + std::to_string(loot_rolls_this_session) + ".txt";
			save_to_file.open(fileoutputname);
			if (!save_to_file) {
				cout << "Could not open to save. Check src or file.\n\n";
				return;
			} else {
				if (treasure_pile.full_gear_list.empty()) {
		...
		if (loot_results_ready) {
			...
			for (int i = 0; i < 5; i++) {
				...
				if (i == 0)
					hoard_menu_display[i].draw(((ScreenWidth / 2) - (hoard_menu_display[i].getWidth() / 2)), 110);
				else
					hoard_menu_display[i].draw(((ScreenWidth / 2) - (hoard_menu_display[i].getWidth() / 2)),
											   110 + i * 75);
			}
		}
